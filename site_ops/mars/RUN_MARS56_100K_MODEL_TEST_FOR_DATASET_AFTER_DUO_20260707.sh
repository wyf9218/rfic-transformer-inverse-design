...
# Usage after Duo is available, for any completed 100k production chunk:
#   REMOTE_DATASET_DIR=/volumes/.../datasets/<chunk_dir> \
#   CHUNK_INDEX=2 \
#   bash RUN_MARS56_100K_MODEL_TEST_FOR_DATASET_AFTER_DUO_20260707.sh
#
# This script deliberately requires an explicit REMOTE_DATASET_DIR so an old
# geometry-only queue cannot be tested by accident.
...
if [ -z "${REMOTE_DATASET_DIR:-}" ]; then
  echo "ERROR: REMOTE_DATASET_DIR is required." >&2
  echo "Example: REMOTE_DATASET_DIR=/volumes/.../datasets/chunk_002_100k_after_chunk08_pass CHUNK_INDEX=2 bash $0" >&2
  exit 2
fi
if [[ "$REMOTE_DATASET_DIR" == *"'"* || "$REMOTE_DATASET_DIR" == *$'\n'* || "$REMOTE_DATASET_DIR" == *$'\r'* ]]; then
  echo "ERROR: REMOTE_DATASET_DIR contains unsupported quote/newline characters." >&2
  exit 2
fi
case "$REMOTE_DATASET_DIR" in
  "$CAMPAIGN_BASE"/datasets/*_100k_after_chunk08_pass) ;;
  *)
    echo "ERROR: REMOTE_DATASET_DIR must be a U8-gated 100k production dataset under ${CAMPAIGN_BASE}/datasets and end with _100k_after_chunk08_pass." >&2
    exit 2
    ;;
esac

CHUNK_INDEX="${CHUNK_INDEX:-unknown}"
if [[ "$CHUNK_INDEX" == *"'"* || "$CHUNK_INDEX" == *$'\n'* || "$CHUNK_INDEX" == *$'\r'* ]]; then
  echo "ERROR: CHUNK_INDEX contains unsupported quote/newline characters." >&2
  exit 2
fi
...
EXPECTED=100000
DATASET="${REMOTE_DATASET_DIR:?REMOTE_DATASET_DIR must be exported}"
CHUNK_INDEX="${CHUNK_INDEX:-unknown}"
...
case "$DATASET" in
  "$BASE"/datasets/*) ;;
  *)
    echo "STATUS=REJECT_DATASET_OUTSIDE_CAMPAIGN_BASE"
    echo "Dataset must be under $BASE/datasets."
    exit 0
    ;;
esac

if [ ! -d "$DATASET" ]; then
  echo "STATUS=WAIT_DATASET_MISSING"
  exit 0
fi
...
printf 'nonempty_s4p=%s\n' "$nonempty"
printf 'empty_stale15=%s\n' "$empty_stale"
printf 'runner=%s\n' "$runner"
printf 'workers=%s\n' "$workers"
printf 'emx=%s\n' "$emx"

if [ "$nonempty" -lt "$EXPECTED" ]; then
  echo "STATUS=WAIT_DATASET_RUNNING_OR_INCOMPLETE"
...
if [ ! -f "$dataset_summary" ]; then
  echo "STATUS=WAIT_DATASET_SUMMARY_MISSING"
  exit 0
fi
...
printf 'dataset_summary=%s\n' "$dataset_summary"
printf 'dataset_summary_status=%s\n' "$dataset_status"
if [ "$dataset_status" != "PASS" ]; then
  echo "STATUS=WAIT_DATASET_SUMMARY_NOT_PASS"
  exit 0
fi
...
echo "STATUS=RUNNING_100K_MODEL_TEST"
...
PY
  ...
  if [ "$proof" = "PASS" ]; then
    echo "STATUS=100K_CHECKPOINT_PROOF_PASS"
  else
    echo "STATUS=100K_CHECKPOINT_PROOF_FAIL"
  fi
else
  echo "checkpoint_summary_missing"
  echo "STATUS=100K_CHECKPOINT_SUMMARY_MISSING"
fi
REMOTE

echo "Opening ${USER_NAME}@${JUMP_HOST}; approve Duo when prompted."
echo "This will test exactly this remote dataset: ${REMOTE_DATASET_DIR}"
ssh "${SSH_ARGS[@]}" "${USER_NAME}@${JUMP_HOST}" \
  "REMOTE_DATASET_DIR='${REMOTE_DATASET_DIR}' CHUNK_INDEX='${CHUNK_INDEX}' ssh -tt ${MARS_HOST} 'REMOTE_DATASET_DIR='\\''${REMOTE_DATASET_DIR}'\\'' CHUNK_INDEX='\\''${CHUNK_INDEX}'\\'' bash -s'" \
  <<<"$REMOTE_RUN"
