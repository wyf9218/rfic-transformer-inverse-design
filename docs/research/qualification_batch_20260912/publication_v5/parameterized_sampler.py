"""Parameterized copy of the frozen successor sampler; no scheduler/native work.

Only explicit input paths, seed pair and batch identity are parameterized.
The neighbor kernel and proposal geometry construction are unchanged.
"""
import argparse
import csv
import hashlib
...
from successor_exclusions import extend_known_subsets, known_duplicate_reasons
...
def main(argv=None):
    ...
    O=args.out
    if not O.is_absolute() or any(p.is_symlink() for p in (O,*O.parents)):
        raise ValueError('Absolute nonsymlink output required')
    if O.exists():
        raise ValueError('NO_CLOBBER no regenerate')
    assert intent['counts']=={'GEOMETRY_DOE':192,'TRAIN_NEIGHBORHOOD':64}
    assert intent['schema']=='eucap15_successor_factory_sampler_intent.v1'
    assert intent['seeds']['split']==17 and set(intent['seeds'])=={'doe','neighbor','split'}
    assert all(type(intent['seeds'][k]) is int and 0<=intent['seeds'][k]<2**32 for k in ('doe','neighbor'))
    assert intent['seeds']['doe']!=intent['seeds']['neighbor']
    assert intent['suggested_new_budget']['maximum_actual_EMX_starts']==256
    assert intent['suggested_new_budget']['maximum_wall_seconds']==43200
    assert intent['suggested_new_budget']['maximum_incremental_bytes']==5368709120
    ...
    old=intent['inputs']
    assert set(old)=={'contract','splits','current_source_rows'}
    ...
    known=intent['known_pool']
    excluded=read(known['path'],known['sha256'])
    ...
    production_known=set(excluded['production_fingerprint_known'])
    ...
    canonical_sets['FROZEN6329_BASELINE']=set()
    grid_sets['FROZEN6329_BASELINE']=set()
    for row in metadata:
        g=[float(row['geom__'+f]) for f in fields]
        h=canonical_geometry_sha256(dict(zip(fields,g)),fields=fields)
        assert h==row['geometry_sha256']
        canonical_sets['FROZEN6329_BASELINE'].add(h)
        gg=_grid_geometry(np.asarray(g),grid_um).tolist()
        grid_sets['FROZEN6329_BASELINE'].add(canonical_geometry_sha256(dict(zip(fields,gg)),fields=fields))
        production_known.update((fingerprint(g),fingerprint(gg)))
    additions=[]
    for parent in intent['prior_candidate_files']:
        rows=[json.loads(line) for line in read(parent['path'],parent['sha256'],text=True).splitlines()]
        cs,gs,ps={},{},set()
        delta=extend_known_subsets(canonical_sets=cs,grid_sets=gs,production_known=ps,
            original_rows=rows,historical_rows=[],fields=fields,grid_um=grid_um,
            canonicalize=canonical_geometry_sha256,
            grid_geometry=lambda g,s:_grid_geometry(np.asarray(g,float),s).tolist(),fingerprint=fingerprint,
            expected_historical_count=0)
        name='PRIOR_BATCH_'+parent['sha256']
        canonical_sets[name]=cs['PREVIOUS256_ALL_RAW_AND_GRID']
        grid_sets[name]=gs['PREVIOUS256_ALL_RAW_AND_GRID']
        production_known.update(ps)
        additions.append(dict(candidate_file=parent,previous_proposals=delta['previous_proposals']))
    exclusion_extension=dict(known_pool=known,prior_batches=additions,
        source_numeric_labels_used_for_exclusions=0,full_history_union_complete=False,
        owner_current_mixed_ledger_check='REQUIRED')
    save('EXCLUSION_METADATA.json',dict(canonical_hashes={k:sorted(v) for k,v in canonical_sets.items()},
        ...
        production_fingerprint_scope='FROZEN6329_PLUS_EXPLICIT_PUBLISHED_KNOWN_POOL_AND_PRIOR_CANDIDATE_DELTAS',
        new_extension=exclusion_extension,published_parent_manifests=intent['published_parent_manifests'],
    ...
    for order,(r,h,ph) in enumerate(zip(proposals,hashes,phashes),1):
        raw_h=canonical_geometry_sha256(dict(zip(fields,r['raw_proposed_geometry'])),fields=fields)
        raw_ph=fingerprint(r['raw_proposed_geometry'])
        reasons=known_duplicate_reasons(canonical=h,raw_canonical=raw_h,production=ph,raw_production=raw_ph,
            canonical_sets=canonical_sets,grid_sets=grid_sets,production_known=production_known)
        ...
        r.update(schema='eucap15_acquisition_candidate.v1',protocol_schema=intent['schema'],
            ...
            canonical_geometry_sha256=h,raw_canonical_geometry_sha256=raw_h,
            nominal_grid_geometry_sha256=h,parameter_geometry_hash=_geometry_hash(r['geometry'],fields),
            production_geometry_fingerprint=ph,raw_production_geometry_fingerprint=raw_ph,
    ...
    save('PREPARATION_RECEIPT.json',dict(status='FROZEN_256_PROPOSALS_NOT_NATIVE_RELEASE',
        ...
        old64_budget_unchanged=True,old256_budget_unchanged=True,new_budget_owner_registration_required=True,
        known_subset_extension=exclusion_extension,actual_owner_latest_mixed_ledger_recheck='REQUIRED',
        successor_auto_dispatch_deployed=False,requested_native_concurrency=48,actual_native_concurrency_authorized_by_this_step=False,
