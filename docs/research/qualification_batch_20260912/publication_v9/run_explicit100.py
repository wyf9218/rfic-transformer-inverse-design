"""One explicit100-row historical partition; unchanged111 formulas, no native action."""
...
def main():
    ...
    if original["sample_count"]!=100 or len(original["rows"])!=100:raise ValueError("not explicit100-member partition")
    if len({row["source_row_index"] for row in original["rows"]})!=100:raise ValueError("duplicate source ordinal")
    ...
    summary=dict(schema="p215_explicit100_actual_historical111_conditional_reextraction.v1",
        started_utc=start,finished_utc=datetime.now(timezone.utc).isoformat(),
        scope=original["scope_label"],evidence_class="HISTORICAL_EM_RESPONSE_CURRENT_MAPPING_DIAGNOSTIC_NOT_FRESH",
        ...
        qualification_caveat=original["qualification_caveat"])
