"""Public path-parameterized reference, NOT the executed native source.

Only the RN path binding differs from the original runner. This file requires
private release/INPUTS/proofs and the native owner; it is not turnkey delivery.
"""
...
RN = Path(os.environ['EUCAP15_NATIVE_OWNER_ROOT'])  # Explicit private binding; no default.
...
def main():
    ...
    expected = inputs['baseline_records']
    a.require(len(before) >= 20 and all(before[i]['path'] == p['path'] and
              before[i]['sha256'] == p['sha256'] for i,p in enumerate(expected)),
              'original twenty commits changed')
    ...
    a.require(all(after[i]['sha256'] == p['sha256'] for i,p in enumerate(before)),
              'a preexisting commit changed')
    a.raw(inputs['continuity']['current_formal_state']['pin'])
    v = os.statvfs(target)
    ...
    receipt = dict(schema='eucap15_existing11_holdout_admission_readback.v3', utc=now(),
        ...
        held=sum(x['status'].startswith('HOLD') for x in outcomes), unprocessed=len(inputs['entries'])-len(outcomes),
        ...
        processed_original_splits=inputs['processed_original_splits'],
        original_twenty_bytes_unchanged=True,
