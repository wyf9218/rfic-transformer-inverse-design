...
def allocated_snapshot(root, candidate_roots):
    """One fresh walk, with inode-deduplicated totals for each exact subtree.

    No cached allocation, symlink traversal, or geometry/solver change. Files
    created after this observation remain subject to the existing in-flight
    reservations; this is not an atomic filesystem snapshot or a peak bound.
    """
    root=slots.path_ok(root)
    indexed={};allocations={rid:0 for rid in candidate_roots}
    seen={rid:set() for rid in candidate_roots};total_seen=set();total=0
    for rid,value in candidate_roots.items():
        ...
        indexed[key]=rid
    lengths={len(key) for key in indexed}
    def fail(error):
        # An unreadable subtree cannot be silently omitted from the budget.
        raise error
    for parent,directories,files in os.walk(root,followlinks=False,onerror=fail):
        for p in [Path(parent),*[Path(parent)/n for n in files+directories]]:
            ...
            m.require(not stat.S_ISLNK(s.st_mode),'BUDGET_TREE_SYMLINK_FORBIDDEN')
            inode=(s.st_dev,s.st_ino);size=s.st_blocks*512
            if inode not in total_seen:
                total_seen.add(inode);total+=size
            parts=p.relative_to(root).parts
            for length in lengths:
                rid=indexed.get(parts[:length]) if len(parts)>=length else None
                if rid is not None and inode not in seen[rid]:
                    seen[rid].add(inode);allocations[rid]+=size
    return total,allocations


class AdmissionTiming:
    """One in-memory observation, emitted only with a grant or terminal event."""

    def __init__(self):
        self.started=time.monotonic();self.attempts=0
        self.lock_wait=0.0;self.lock_hold=0.0;self.holding_since=None
        self.rejections={}
    ...
    @contextmanager
    def dispatch(self,lock):
        began=time.monotonic()
        with lock:
            acquired=time.monotonic()
            self.lock_wait+=acquired-began;self.holding_since=acquired
            try:yield
            finally:
                self.lock_hold+=time.monotonic()-acquired
                self.holding_since=None

    def summary(self):
        at=time.monotonic()
        active=at-self.holding_since if self.holding_since is not None else 0.0
        return dict(schema='eucap15_admission_timing.v1',clock='monotonic',
            total_wait_seconds=at-self.started,attempts=self.attempts,
            dispatch_lock_wait_seconds=self.lock_wait,
            dispatch_lock_hold_seconds_through_summary=self.lock_hold+active,
            rejection_counts=dict(self.rejections),
            timing_cut='BEFORE_EXISTING_EVENT_WRITE_EXCLUDES_EVENT_FSYNC_AND_FINAL_UNLOCK')
...
class Manager:
    def __init__(self, owner):
        ...
        self.permits={};self.permit_locks={};self.last_pin=None
        self.admission_terminal_timings={}
    ...
    def permit_lock(self,rid):
        # Order: dispatch -> candidate permit -> state. Release skips dispatch;
        # state-only readers never acquire either outer lock or perform I/O.
        with self.lock:return self.permit_locks.setdefault(rid,threading.RLock())
    ...
    def storage_snapshot(self):
        ...
        total,allocations=allocated_snapshot(self.config['budget_root'],
            {rid:self.owner.out/rid for rid in self.storage.claims})
        ...
        record=dict(schema='eucap15_fixed48_inflight_storage.v1',binding=self.binding,utc=clock(),
            ...
            allocation_scan=dict(method='SINGLE_FRESH_ROOT_WALK',traversals=1,
                claimed_roots=len(allocations),elapsed_seconds=scan_seconds))
    ...
    def require_ready(self):
        ...
        # The next admission decision uses this same fresh full-root snapshot.
        state=self.storage_snapshot()
        if state['total_allocated_bytes']>=m.MAX_INCREMENTAL_BYTES:
    ...
    def candidate(self,job):
        ...
        while True:
            with self.dispatch_lock:
                state=self.require_ready()
                if self.storage.claim(rid,e.allocated_bytes(self.owner.out/rid),state['total_allocated_bytes'],
                        self.policy['candidate_projected_peak_bytes']):
                    self.storage_snapshot()
                    break
            # The earlier dispatch estimate is not a reservation. Wait outside
            # the dispatch lock so active candidates can finish and release it.
    ...
    def acquire(self,root,tool):
        ...
        try:return self._acquire(root,tool,timing)
        except BaseException as error:
            timing.reject(type(error).__name__+': '+str(error))
            with self.lock:
                self.admission_terminal_timings[root.name]=dict(tool=tool,**timing.summary())
            raise

    def _acquire(self,root,tool,timing):
        rid=root.name
        self.release(rid)
        while True:
            timing.attempts+=1
            ...
            if decision['additional'][tool]>0:
                with timing.dispatch(self.dispatch_lock):
                    ...
                    if decision['additional'][tool]>0:
                        ...
                        self.require_wait_ready()
                        if fits:self.owner.ensure_plan(self.last_pin)
                        with self.permit_lock(rid):
                            with self.lock:
                                ...
                                if fits and decision['additional'][tool]>0 and not self.fault:
                                    m.require(rid not in self.permits,'CANDIDATE_ALREADY_HAS_TOOL_PERMIT')
                                    ...
                                    value=None
                                    if not fits:timing.reject('PROJECTED_STORAGE_OVER_BUDGET')
                                    if decision['additional'][tool]<=0:timing.reject('resource_final: '+decision['reason'])
                                    if self.fault:timing.reject('FIXED48_SHARED_RESOURCE_FAULT: '+self.fault)
                            if value is not None:
                                ...
                                if eligible:
                                    self.owner.event('FIXED48_TOOL_PERMIT_GRANTED',request=rid,tool=tool,permit=e.pin(p),
                                        ...
                                        admission_timing=timing.summary())
                                    return
                                ...
                                timing.reject('FIXED48_PERMIT_REVOKED_BEFORE_TOOL_START')
                    else:timing.reject('resource_locked: '+decision['reason'])
            else:timing.reject('resource_outer: '+decision['reason'])
            if self.stop.wait(2):raise RuntimeError('FIXED48_OWNER_DRAINING')

    def release(self,rid):
        with self.permit_lock(rid):
            with self.lock:value=self.permits.get(rid)
            if value is not None:
                try:
                    ...
                    with self.lock:self.fault='TOOL_PERMIT_RELEASE_PERSISTENCE: '+type(error).__name__+': '+str(error)
                    raise
                # Keep counting the permit until its inactive marker is durable.
                # Candidate storage is released only by candidate() after all QA.
                with self.lock:
                    m.require(self.permits.get(rid) is value,'TOOL_PERMIT_CHANGED_DURING_RELEASE')
                    del self.permits[rid]
