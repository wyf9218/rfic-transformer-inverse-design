"""Append existing, individually evidenced train members; no simulation API."""
import csv
import hashlib
import io
import json
...
def validate_shared(inputs):
    ...
    require(len(entries) == 19 and len({e['member']['candidate_id'] for e in entries}) == 19,
            'wrong bounded train partition')
    require(all(e['member']['split'] == 'train' for e in entries), 'validation/test outside scope')
    require(len({e['member']['geometry_sha256'] for e in entries}) == 19, 'duplicate partition geometry')
...
def qualify(entry, inputs, formal, reader=raw):
    ...
    require(m['split'] == c['fixed_split'] == gap['split'] == 'train', 'split drift')
...
def publish(root, evidence, utc, *, first_sha=FIRST_SHA, verify):
    ...
    require(not any(p.is_symlink() for p in (root, *root.parents, root/'records', root/'WRITE.lock')),
            'symlink destination')
    require(evidence['member']['split'] == 'train' and evidence['scientific_contract_fingerprint'] == FP and
            evidence['qualification'] == 'PASS_RECORDED_CHAIN_AND_CURRENT_BYTE_IDENTITIES_NOT_NEW_PHYSICAL_QA',
            'not an eligible train qualification')
    with lease(root/'WRITE.lock'):
        ...
        verify(evidence)
        seq = len(committed)+1
        value = dict(schema='eucap15_bounded_train_qualified_increment.v2', status=STATUS, utc=utc,
