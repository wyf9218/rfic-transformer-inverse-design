...
最新完整统计现场：2026-09-12 12:29:42.606254 UTC（不是之后持续存活保证）。

- 11:59:12→12:29:42：135新fresh EMX，63新范围内strict均已正式；另原2pending补登，fresh正式共65、历史0；pending0，认证下界6736。
- 申请48／容量48／实跑12EMX×2CPU／当前许可14；batch000002自然终态，000003已自动真实启动。
- 12:28:45已预约安全交接：旧等待installer已结束，唯一续接installer3265510活，仅standby下一批派发暂留；当前owner/metadata/健康solver继续。
- 23eddf仍未实际安装；普通程序等当前批自然排空后安装并接batch4，不手动重启/重复派发。
- 12:08隔离失败→12:13五个稳定样本恢复；低并发不能只归预算walk，不关闭隔离检查。
- [最新事实与证据](production_increment_m24_20260912/README.md)；[最终观察截面](production_increment_m24_20260912/FINAL_SNAPSHOT.json)。
- 同一离线入口实际续接41条train，已接收来源并集3865→3906，覆盖163/512不变；不是完整生产覆盖。
- 历史下一分区仅定位offset，不计新增认证；工艺/端口/版本缺证不能凭再读标签填PASS。
