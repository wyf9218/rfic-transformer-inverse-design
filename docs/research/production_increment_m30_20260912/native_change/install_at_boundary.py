...
def sync_boundary_registry(old,root):
    """Preserve late frozen-input registrations before retiring the old waiter."""
    copied=[]
    for source in sorted((old/'batch_registry').glob('*.json')):
        identity=e.pin(source);raw=m.read_pin(identity)
        target=root/'batch_registry'/source.name
        if target.exists():
            m.require(target.read_bytes()==raw,'STAGED_REGISTRY_CONFLICT')
        else:
            with target.open('xb') as stream:
                stream.write(raw);stream.flush();os.fsync(stream.fileno())
        actual=e.pin(target)
        m.require(actual['sha256']==identity['sha256'] and actual['bytes']==identity['bytes'],
            'BOUNDARY_REGISTRY_COPY_CHANGED')
        copied.append(dict(source=identity,copy=actual))
    return copied
    ...
    with global_lease(old/'PERMIT_RELEASE_UPGRADE.lock'):
        ...
        while time.monotonic()<deadline:
            ...
            try:
                ...
                registry=sync_boundary_registry(old,root)
                registered=e.document(release['successor_registration']['inputs'])
                m.require(c.reg.registered_profile(config['original_manifest'])==registered['profile'],
                ...
                s.write_once(root/'BOUNDARY_PREFLIGHT.json',dict(utc=e.now(),status='PASS',
                    ...
                    source_tests=staged['tests'],registry=registry,closure=closure,
                    native_actions=0,physical_qa_recomputed=0))
