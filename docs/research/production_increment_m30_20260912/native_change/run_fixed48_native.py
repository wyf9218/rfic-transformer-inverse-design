...
class Owner(SerialOwner):
    def __init__(self, path):
        super().__init__(path)
        m.require(self.release.get('concurrency_amendment') or self.release.get('successor_registration'),'FIXED48_AUTHORITY_REQUIRED')
        ...
        if self.release.get('successor_registration'):
            ...
            self.ledger=Ledger(self.release['successor_registration']['formal_ledger'])

    def event(self,status,**data):
        manager=getattr(self,'manager',None)
        timings={}
        if manager is not None and status in ('NEW_RESULT_CAPTURED','FIXED48_OWNER_EXIT_NO_CHILD_SIGNAL'):
            with manager.lock:
                if status=='NEW_RESULT_CAPTURED':
                    rid=data['request'];timing=manager.admission_terminal_timings.get(rid)
                    timings={rid:timing} if timing is not None else {}
                else:
                    timings=dict(manager.admission_terminal_timings)
            if timings:data['admission_terminal_timings']=timings
        result=super().event(status,**data)
        if timings:
            with manager.lock:
                for rid,timing in timings.items():
                    if manager.admission_terminal_timings.get(rid) is timing:
                        del manager.admission_terminal_timings[rid]
        return result

    def admit(self, root, stage):
        self.manager.acquire(root,stage)
        if self.ledger is not None and stage=='emx':
            from live_dedup import CommittedDuplicate
            from fixed48_runtime import suffix
            value=self.ledger.check(self.batch.candidate(root.name)['original'])
            path=root/('PRE_EMX_DEDUP_'+suffix()+'.json');write(path,value)
            if value['matches']:raise CommittedDuplicate(self.pin(path))
    ...
    def one(self,job):
        ...
        with self.manager.candidate(job):
            ...
            elif job['request_id'] in self.config.get('recovery_unresolved',{}):
                value=results.failed_candidate(self.batch,job['request_id'],
                    error='RECOVERY_PARTIAL_PRESERVED_NO_AUTOMATIC_TOOL_REPEAT',
                    stage_evidence=self.config['recovery_unresolved'][job['request_id']])
            else:
                value=self.budget_decision(job)
                if value is None:
                    try:value=self.candidate(job)
                    except __import__('live_dedup').CommittedDuplicate as error:
                        value=results.intake_hold(self.batch,job['request_id'],error.pin)
                    except BudgetEnded:
                        value=self.budget_decision(job)
    ...
    def run(self):
        ...
        with ExitStack() as stack:
            ...
            prior={Path(p['path']).parent.name:p for p in self.release.get('concurrency_amendment',{}).get('prior_results',[])}
...
def main():
    ...
    if args.preflight_only:
        ...
        if owner.binding:execution.load_execution(owner.binding,owner.batch)
        else:m.require(owner.release.get('successor_registration'),'ORIGINAL_BUDGET_PLAN_REQUIRED')
