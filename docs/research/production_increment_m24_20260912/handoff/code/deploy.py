...
out.mkdir()
root = RN+'/continuous_budget_walk_20260912T112357738265Z'
if mode == 'stage':
...
s.write_once(op/'INTEGRATION_TEST.json',dict(utc=e.now(),status='PASS' if result.returncode==0 else 'FAIL',
 ...
 actual_private_python=str(Path(sys.executable).resolve()),unique_integration_tests=1,
...
def precheck():
 ...
 assert set(fds.values())<={'/dev/null',str(root/'BOUNDARY_INSTALLER.log'),str(old/'PERMIT_RELEASE_UPGRADE.lock')},'INSTALLER_HOLDS_UNEXPECTED_FD'
