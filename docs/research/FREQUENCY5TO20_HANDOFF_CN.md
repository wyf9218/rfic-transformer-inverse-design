...
[离线导师报告 v2](frequency5to20_report_20260908_v2/report.html)及[可供GPT审查的结构化内容](frequency5to20_report_20260908_v2/artifact.json)仍为较早两个物理个案/17求解，快照与当前87求解分开；结构和实际源数据通过校验，但浏览器布局验证尚未完成。新可编辑PPT另用固定首6物理/51求解制作，不把报告冻结等同停止健康研究。源码/数据/模型记录以对应SHA和私有入口为准，不把公开仓库当成含私有权重。
