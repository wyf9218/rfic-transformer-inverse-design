...
ENDPOINT_V2_UPGRADE_SHA='5ab6cf17dca566e4bd0a0bca905b76d09a1d9e2d1fe5d2acebfcab91aecba638'
ENDPOINT_V2_REGISTRATION_SHA='27f3f0822db9654f692764efa4616a018db176a5b953a32859644d90ac8faa4a'
ENDPOINT_RECEIVER_SHA='b8f3adf8447475783b2404e914d90de299be7db29e743a16be28366c40ead43a'
STATE_SUFFIX='_endpointv2closure_v1'
...
def family_library(bridge,args,callback):
    ...
    base=load_pinned('exact_endpoint_v2_receiver',args.endpoint_receiver,ENDPOINT_RECEIVER_SHA)
    ...
    with patch.object(bridge,'library',load),patch.object(prepared,'c',callback),\
         patch.object(callback,'RECEIVER_SHA',ENDPOINT_RECEIVER_SHA):
        yield
...
def successor_registration_anchor(fixed,successor,release):
    """Admit only the deployed v2 registry tied to its exact frozen upgrade."""
    binding=release['successor_registration']
    upgrade=binding.get('endpoint_upgrade')
    if upgrade is None:return successor.TRUSTED_REGISTRATION_SHA
    require(upgrade.get('sha256')==ENDPOINT_V2_UPGRADE_SHA,
            'endpoint upgrade identity is not frozen for this metadata adapter')
    fixed.bytes_for(upgrade)
    config=fixed.read(release['config']);root=Path(config['code_root'])
    source={p['path']:p for p in release['sources']}.get(str(root/'batch_registration.py'))
    require(source is not None and source['sha256']==ENDPOINT_V2_REGISTRATION_SHA,
            'endpoint-v2 registration source identity changed')
    fixed.bytes_for(source)
    return ENDPOINT_V2_REGISTRATION_SHA
...
def decorate(fixed,args,profile,callback):
    ...
    expected='native_candidate_publication_family_v3_'+rp['sha256'][:12]+STATE_SUFFIX
    require(Path(args.state).name==expected,'new release-scoped family_v3 state required')
    pins=[fixed.pin(__file__),fixed.pin(args.successor_entry),fixed.pin(callback.__file__),fixed.pin(args.endpoint_receiver)]
    require(pins[1]['sha256']==SUCCESSOR_SHA and pins[2]['sha256']==CALLBACK_SHA and pins[3]['sha256']==ENDPOINT_RECEIVER_SHA,
    ...
    profile['family_publication']=dict(schema='eucap15_family_callback_binding.v3',
        wrapper=pins[0],successor_reader=pins[1],actual_callback=pins[2],actual_endpoint_receiver=pins[3],
...
def rebind_state(bridge,ctx,args,profile):
    ...
    require(ctx.state.name==legacy and actual.parent==ctx.union.parent and
            actual.name=='native_candidate_publication_family_v3_'+profile['release']['sha256'][:12]+STATE_SUFFIX and
...
def execute(args):
    ...
    require(Path(args.state).name=='native_candidate_publication_family_v3_'+rp['sha256'][:12],
            'original release-scoped metadata worker lock required')
    args=copy.copy(args)
    # Keep the worker's existing release lock; version only receiver cache/state.
    args.state=Path(str(args.state)+STATE_SUFFIX)
    ...
    with family_library(bridge,args,callback):
        ...
        if release.get('successor_registration') is not None:
            runtime,amendment_sha=recovery_anchors(fixed,release)
            registration_sha=successor_registration_anchor(fixed,successor,release)
            # Preserve d5ca's original successor runtime for its original
            # amendment; only the known recovery uses the new frozen tuple.
            if amendment_sha==successor.AMENDMENT_SOURCE_SHA:
                runtime=successor.TRUSTED_SUCCESSOR_RUNTIME
            with patch.object(successor,'AMENDMENT_SOURCE_SHA',amendment_sha),\
                 patch.object(successor,'TRUSTED_SUCCESSOR_RUNTIME',runtime),\
                 patch.object(successor,'TRUSTED_REGISTRATION_SHA',registration_sha):
...
def main():
    ...
    for name in ('successor-entry','fixed48-entry','legacy-bridge','family-callback','library-dir',
                 'release','contract-inputs','state','endpoint-receiver'):
