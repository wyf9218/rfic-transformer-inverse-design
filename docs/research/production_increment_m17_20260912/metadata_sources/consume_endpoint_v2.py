...
ENDPOINT_V2_SOURCE_SHA={
 'evaluator.py':'3bf7f1d11f74b85e007f5bf964bec8f19379d708ab5bc850b90c122b08ac30a9',
 'export.py':'7030d03fe08806ff1a8b5e3a34dddc8471c135659fe7d9c783613e1058f0e8e7',
 'port_endpoint_lineage.py':'7d0e0a67aed3364cc4cbcdf2ea2bc6121863eb4184d48cc50f0ec8abd9536061',
 'run_candidate_queue_dataset.py':'3c908101249cbdb7e7644352e696ab102f75c5269fce5fc8121c7dbb6e8b884c',
 'run_candidate_queue_dataset_parallel.py':'0b0341ecec11d6049e9a3de95899ed3cb576e30b4783b9a467de66bf837f3020',
}


def validate_endpoint_sources(active,reads):
    upgrade_pin=active.get('successor_registration',{}).get('endpoint_upgrade')
    if upgrade_pin is None:
        require(len(active['endpoint_sources'])==6,'EXACT_SOURCE6_CLOSURE_REQUIRED')
    else:
        require(upgrade_pin['sha256']==ENDPOINT_V2_UPGRADE_SHA,'UNTRUSTED_ENDPOINT_UPGRADE')
        upgrade=reads.load(upgrade_pin)
        require(active['endpoint_sources']==list(upgrade['changes'].values()),'ENDPOINT_UPGRADE_SOURCE_BINDING_CHANGED')
        items=active['endpoint_sources']
        require(len(items)==5 and {Path(p['new']['path']).name:p['new']['sha256'] for p in items}==ENDPOINT_V2_SOURCE_SHA,
                'EXACT_ENDPOINT_V2_SOURCE5_CLOSURE_REQUIRED')
...
def receive_one(spec,entry,batch,metadata,result,execution,reads):
    ...
    validate_endpoint_sources(active,reads)
