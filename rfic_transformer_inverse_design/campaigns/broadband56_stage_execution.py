    ...
    *,
    ...
    for stage in sorted(expected_stage_names):
        ...
        if "max_candidates_per_attempt" in stage_profile:
            ...
            if stage == "GOLDEN" or type(limit) is not int or not 1 <= limit <= 192:
                errors.append(f"stages.{stage}.max_candidates_per_attempt must be 1..192 outside GOLDEN")
            ...
            if isinstance(argv, list) and "--reuse-campaign-frozen-cohort" in argv:
                if (stage not in {"PILOT_1000", "PHASE_A"}
                        or argv.count("--reuse-campaign-frozen-cohort") != 1
                        or any(flag in argv for flag in ("--frozen-queue-receipt", "--frozen-queue-receipt-sha256"))):
                    errors.append(f"stages.{stage} campaign cohort mode is ambiguous or unsupported")
            else:
