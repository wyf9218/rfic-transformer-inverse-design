#!/usr/bin/env python3
"""Audit the resilient Windows runner for the HFSS V67 material/mesh sweep."""
...
DEFAULT_PLAN_DIR = PROJECT_ROOT / "outputs" / "hfss_v67_material_mesh_calibration_plan_current"
DEFAULT_PLAN_SUMMARY = DEFAULT_PLAN_DIR / "hfss_v67_material_mesh_calibration_plan_summary.json"
DEFAULT_RUNNER = DEFAULT_PLAN_DIR / "run_hfss_v67_material_mesh_calibration_resilient.windows.ps1"
DEFAULT_CMD_LAUNCHER = DEFAULT_PLAN_DIR / "run_hfss_v67_material_mesh_calibration_resilient.windows.cmd"
DEFAULT_STATUS = DEFAULT_PLAN_DIR / "resilient_run_status" / "hfss_v67_resilient_run_status.json"
DEFAULT_OUT_DIR = PROJECT_ROOT / "outputs" / "hfss_v67_material_mesh_runner_audit_current"
...
def main(argv: list[str] | None = None) -> int:
    ...
    plan_summary_path = Path(args.plan_summary).expanduser().resolve()
    ...
    plan = _read_json(plan_summary_path)
    status = _read_json(status_path)
    exports = _filesystem_exports(runner.parent)
    variant_catalog = _variant_catalog(plan)
    pass_variant_names = _pass_variant_names(status)
    final_candidate_pass_count = sum(
        1 for name in pass_variant_names if variant_catalog.get(name, {}).get("final_acceptance_candidate") is True
    )
    diagnostic_only_pass_count = sum(
        1 for name in pass_variant_names if variant_catalog.get(name, {}).get("diagnostic_only") is True
    )
    checks = _checks(plan_summary_path, plan, runner, cmd_launcher, status_path, status, exports, args)
    overall_status = "FAIL" if any(item["status"] == "FAIL" for item in checks) else "PASS"
    decision = _decision(overall_status, status_path, status, final_candidate_pass_count, diagnostic_only_pass_count)
    summary = {
        ...
        "expected_variant_count": _expected_variant_count(plan, status),
        "pass_count": _int_value(status.get("pass_count")) if isinstance(status, dict) else 0,
        "final_candidate_pass_count": final_candidate_pass_count,
        "diagnostic_only_pass_count": diagnostic_only_pass_count,
        "pass_variant_names": pass_variant_names,
        ...
        "checks": checks,
        "method_notes": [
            "PASS without a V67 status JSON means the V67 resilient runner is ready but has not been run in Windows yet.",
            "A completed V67 run requires at least one variant-level .s8p and export manifest.",
            "Any exported .s8p found under variants/ is parsed locally and must match the 8-port, 5-60 GHz, 1.0 GHz, 56-point contract.",
            "The skip-pin variant is diagnostic only; it cannot unlock the million-sample EMX campaign by itself.",
            "This audit does not replace the EMX/HFSS <=10% postrun validation gate.",
        ],
    }
    summary_path = out_dir / "hfss_v67_material_mesh_runner_audit_summary.json"
    report_path = out_dir / "HFSS_V67_MATERIAL_MESH_RUNNER_AUDIT_CN.md"
...
def _checks(
    ...
    expected_variant_count = _expected_variant_count(plan, status)
    checks = [
        _check("V67 plan summary exists", plan_summary_path.is_file(), str(plan_summary_path)),
        _check("V67 plan summary PASS", plan.get("overall_status") == "PASS", str(plan.get("overall_status"))),
        _check("V67 variants exist", len(plan.get("variants") or []) >= 1, str(len(plan.get("variants") or []))),
        _check("V67 runner exists", runner.is_file(), str(runner)),
        _check("V67 cmd launcher exists", cmd_launcher.is_file(), str(cmd_launcher)),
        ...
        _check("cmd launcher calls V67 runner", "run_hfss_v67_material_mesh_calibration_resilient.windows.ps1" in cmd_text, "V67 runner"),
        _check("V67 runner starts transcript", "Start-Transcript" in runner_text, "Start-Transcript"),
        _check("V67 runner writes JSON status", "hfss_v67_resilient_run_status.json" in runner_text and "ConvertTo-Json" in runner_text, "status JSON"),
        _check("V67 runner has per-variant function", "function Run-V67Variant" in runner_text, "Run-V67Variant"),
        _check(
            "V67 runner call count matches variants",
            runner_text.count("$VariantResults += Run-V67Variant") == expected_variant_count,
            f"calls={runner_text.count('$VariantResults += Run-V67Variant')}, expected={expected_variant_count}",
        ),
        _check("V67 runner uses PowerShell backtick continuation", "Run-V67Variant `" in runner_text, "Run-V67Variant `"),
        _check("V67 runner has no shell backslash continuation", "Run-V67Variant \\" not in runner_text, "no Run-V67Variant \\"),
        _check("V67 runner requires exports", "Variant completed but did not produce both .s8p and export manifest." in runner_text, "export guard"),
    ]
    if status_path.is_file():
        ...
        checks.append(
            _check(
                "status has recognized state",
                overall in {"PARTIAL_OR_FULL_EXPORTS_READY_FOR_POSTRUN", "FAIL_NO_EXPORTS"},
                overall,
            )
        )
        if overall == "PARTIAL_OR_FULL_EXPORTS_READY_FOR_POSTRUN":
            pass_count = _int_value(status.get("pass_count"))
            checks.append(_check("completed V67 run has passing variant", pass_count >= 1, f"pass_count={pass_count}"))
            checks.append(
                _check(
                    "completed V67 run has filesystem S8P",
            ...
            checks.append(
                _check(
                    "completed V67 run has export manifest",
        ...
        elif overall == "FAIL_NO_EXPORTS":
            checks.append(_check("completed V67 run produced at least one passing variant", False, overall))
    return checks
...
def _decision(
    ...
    status_path: Path,
    status: dict[str, Any],
    final_candidate_pass_count: int,
    diagnostic_only_pass_count: int,
) -> str:
    if overall_status == "FAIL":
        if str(status.get("overall_status") or "") == "PARTIAL_OR_FULL_EXPORTS_READY_FOR_POSTRUN":
            return "FIX_V67_RUNNER_COMPLETED_WITH_INVALID_EXPORTS"
        if str(status.get("overall_status") or "") == "FAIL_NO_EXPORTS":
            return "FIX_V67_WINDOWS_FAILURES_OR_NO_EXPORTS"
        return "FIX_V67_RUNNER_PACKET"
    if not status_path.is_file():
        return "V67_RUNNER_READY_NOT_YET_RUN"
    if str(status.get("overall_status") or "") == "PARTIAL_OR_FULL_EXPORTS_READY_FOR_POSTRUN":
        if final_candidate_pass_count >= 1:
            return "V67_RUNNER_COMPLETED_RUN_POSTRUN_VALIDATION"
        if diagnostic_only_pass_count >= 1:
            return "V67_RUNNER_COMPLETED_DIAGNOSTIC_ONLY_NOT_FINAL"
        return "V67_RUNNER_COMPLETED_UNKNOWN_VARIANT_EXPORTS"
    return "V67_RUNNER_READY"
...
def _variant_catalog(plan: dict[str, Any]) -> dict[str, dict[str, Any]]:
...
def _pass_variant_names(status: dict[str, Any]) -> list[str]:
    names: list[str] = []
    variants = status.get("variants") if isinstance(status, dict) else None
    if not isinstance(variants, list):
        return names
    for item in variants:
        if isinstance(item, dict) and str(item.get("status") or "") == "PASS" and item.get("name"):
            names.append(str(item["name"]))
    return names


def _expected_variant_count(plan: dict[str, Any], status: dict[str, Any]) -> int:
    for value in (status.get("expected_variant_count"), plan.get("variant_count")):
...
def _render_report(summary: dict[str, Any]) -> str:
    lines = [
        "# HFSS V67 Material/Mesh Runner Audit",
        "",
        ...
        f"- Final-candidate pass count: `{summary['final_candidate_pass_count']}`",
        f"- Diagnostic-only pass count: `{summary['diagnostic_only_pass_count']}`",
        "",
        "## Checks",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Next Action",
            "",
            "- If decision is `V67_RUNNER_READY_NOT_YET_RUN`, run the V67 `.cmd` launcher in Windows/HFSS.",
            "- If decision is `V67_RUNNER_COMPLETED_RUN_POSTRUN_VALIDATION`, run the V67 postrun validator to compare EMX and HFSS physical curves.",
            "- If decision is `V67_RUNNER_COMPLETED_DIAGNOSTIC_ONLY_NOT_FINAL`, use the result only to diagnose fixture sensitivity; it cannot unlock production data.",
        ]
    )
    return "\n".join(lines) + "\n"
