#!/usr/bin/env python3
"""Watch HFSS V67 material/mesh results and advance the million-sample gate.

This local watcher does not run HFSS, EMX, ADS, Guacamole, or SSH. It runs the
local V67 runner audit, then the V67 postrun validator, and only invokes the
million-sample planner after a non-diagnostic full 5-60 GHz HFSS `.s8p` passes
the EMX/HFSS Lp/Ls/Q/K/Kw gate.
"""
...
DEFAULT_V67_PLAN = PROJECT_ROOT / "outputs" / "hfss_v67_material_mesh_calibration_plan_current" / "hfss_v67_material_mesh_calibration_plan_summary.json"
DEFAULT_V67_POSTRUN = PROJECT_ROOT / "outputs" / "hfss_v67_material_mesh_calibration_plan_current" / "postrun_validate_hfss_v67_material_mesh_calibration.sh"
DEFAULT_V67_AUDIT_OUT = PROJECT_ROOT / "outputs" / "hfss_v67_material_mesh_runner_audit_current"
DEFAULT_OUT_DIR = PROJECT_ROOT / "outputs" / "hfss_v67_material_mesh_to_million_gate_watch_current"
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = {
        ...
        "decision": latest.get("decision", "WAIT_FOR_V67_HFSS_S8P"),
        "out_dir": str(out_dir),
        "attempt_count": len(attempts),
        "attempts": attempts,
        "latest": latest,
        "arguments": {
            "v67_plan_summary": str(Path(args.v67_plan_summary).expanduser()),
            "v67_postrun_script": str(Path(args.v67_postrun_script).expanduser()),
            "v67_runner_audit_script": str(Path(args.v67_runner_audit_script).expanduser()),
            "v67_runner_audit_out_dir": str(Path(args.v67_runner_audit_out_dir).expanduser()),
            "skip_v67_runner_audit": bool(args.skip_v67_runner_audit),
        ...
        "method_notes": [
            "HFSS must be run by the Windows/HFSS V67 runner before this watcher can pass.",
            "The V67 runner audit is a preflight gate before postrun and million planning.",
            "A WAITING_FOR_HFSS state is expected when exported HFSS `.s8p` files are absent.",
            "The million planner is invoked only after a non-diagnostic V67 full-band postrun summary passes.",
            "The skip-pin V67 variant can diagnose fixture sensitivity but cannot unlock the million-sample campaign.",
            "This watcher does not fabricate curves, labels, S-parameters, or HFSS results.",
        ],
    }
    summary_path = out_dir / "hfss_v67_material_mesh_to_million_gate_watch_summary.json"
    report_path = out_dir / "HFSS_V67_MATERIAL_MESH_TO_MILLION_GATE_WATCH_CN.md"
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("--v67-postrun-script", default=str(DEFAULT_V67_POSTRUN))
    parser.add_argument("--v67-runner-audit-script", default=str(SCRIPT_DIR / "audit_hfss_v67_material_mesh_runner_status.py"))
    parser.add_argument("--v67-runner-audit-out-dir", default=str(DEFAULT_V67_AUDIT_OUT))
    parser.add_argument("--skip-v67-runner-audit", action="store_true")
...
def _run_once(args: argparse.Namespace) -> dict[str, Any]:
    plan_path = Path(args.v67_plan_summary).expanduser().resolve()
    postrun_path = Path(args.v67_postrun_script).expanduser().resolve()
    plan = _read_json(plan_path)
    audit_result = _run_v67_runner_audit(args)
    audit_summary = _read_json(Path(str(audit_result.get("summary_path") or "")))
    if not args.skip_v67_runner_audit and str(audit_summary.get("overall_status") or "") == "FAIL":
        return {
            "overall_status": "FAIL",
            "decision": "V67_RUNNER_AUDIT_FAILED",
            "v67_plan_summary": str(plan_path),
            "v67_postrun_script": str(postrun_path),
            "v67_runner_audit_result": audit_result,
            "v67_runner_audit_summary": audit_summary,
            "postrun_result": {},
            "variant_count": 0,
...
def _run_v67_runner_audit(args: argparse.Namespace) -> dict[str, Any]:
    audit_out_dir = Path(args.v67_runner_audit_out_dir).expanduser().resolve()
    summary_path = audit_out_dir / "hfss_v67_material_mesh_runner_audit_summary.json"
    if args.skip_v67_runner_audit:
        return {
            "returncode": 0,
            "command": [],
            "stdout_tail": "",
    ...
    script = Path(args.v67_runner_audit_script).expanduser().resolve()
    ...
    command = [
        ...
        str(script),
        "--plan-summary",
        str(Path(args.v67_plan_summary).expanduser().resolve()),
        "--out-dir",
        str(audit_out_dir),
        "--no-fail-exit",
    ]
        ...
        records.append(
            {
                ...
                "variant_final_acceptance_candidate": bool(item.get("final_acceptance_candidate")),
...
def _read_variant_summary(variant: dict[str, Any]) -> dict[str, Any]:
    ...
    postrun_final = bool(summary.get("final_acceptance_candidate"))
    variant_final = bool(variant.get("variant_final_acceptance_candidate"))
    diagnostic_only = bool(variant.get("diagnostic_only"))
    return {
        "name": variant["name"],
        "diagnostic_only": diagnostic_only,
        "variant_final_acceptance_candidate": variant_final,
        ...
        "postrun_final_acceptance_candidate": postrun_final,
        "final_acceptance_candidate": bool(postrun_final and variant_final and not diagnostic_only),
...
def _decision(
    ...
    if str(plan.get("overall_status") or "") not in {"PASS", ""}:
        return "FAIL", "V67_PLAN_NOT_READY"
    ...
    if campaign_status == "FAIL":
        return "FAIL", "MILLION_CAMPAIGN_GATE_FAILED_AFTER_V67_PASS"
    if selected:
        return "WAITING_FOR_CAMPAIGN_PLANNER", "V67_HFSS_GATE_PASSED_RUN_MILLION_PLANNER"
    if not variants:
        return "FAIL", "NO_V67_VARIANTS_FOUND"
    final_candidate_statuses = {
        str(item.get("overall_status"))
        for item in variants
        if not item.get("diagnostic_only") and item.get("variant_final_acceptance_candidate") is True
    }
    if "WAITING_FOR_HFSS" in final_candidate_statuses or "MISSING" in final_candidate_statuses:
        return "WAITING_FOR_HFSS", "WAIT_FOR_V67_EXPORTED_HFSS_S8P"
    diagnostic_passes = [
        item for item in variants if item.get("diagnostic_only") and item.get("overall_status") == "PASS"
    ]
    if diagnostic_passes:
        return "FAIL", "ONLY_V67_DIAGNOSTIC_VARIANTS_PASSED_DO_NOT_UNLOCK_MILLION"
    if final_candidate_statuses and final_candidate_statuses <= {"FAIL"}:
        return "FAIL", "ALL_V67_FINAL_CANDIDATE_VARIANTS_FAILED_EMX_HFSS_GATE"
    return "WAITING_FOR_HFSS", "WAIT_FOR_V67_EXPORTED_HFSS_S8P"
...
def _render_report(summary: dict[str, Any]) -> str:
    ...
    lines = [
        "# HFSS V67 Material/Mesh To Million Gate Watch",
        "",
        f"- Overall status: **{summary['overall_status']}**",
        f"- Decision: `{summary['decision']}`",
        f"- Attempts: `{summary.get('attempt_count')}`",
        f"- V67 variants: `{latest.get('variant_count', 0)}`",
    ...
    if summary["overall_status"] == "PASS":
        lines.append("V67 full-band EMX/HFSS gate passed for a non-diagnostic variant and the million-sample campaign planner is ready.")
    elif summary["overall_status"] == "FAIL":
        lines.append("V67 did not produce an acceptable non-diagnostic full-band EMX/HFSS gate result. Do not start production EMX generation.")
    else:
        lines.append("Waiting for exported HFSS `.s8p` files or a passing V67 postrun result. Million-sample generation remains locked.")
    lines.extend(
        [
            "",
